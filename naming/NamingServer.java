...
public class NamingServer implements Service, Registration {
	...
	public synchronized void start() throws RMIException {
		...
		serviceSkeleton = new Skeleton<>(Service.class, new NamingServer(), new InetSocketAddress("127.0.0.1", NamingStubs.SERVICE_PORT));
		registrationSkeleton = new Skeleton<>(Registration.class, new NamingServer(), new InetSocketAddress("127.0.0.1", NamingStubs.REGISTRATION_PORT));
	...
	public String[] list(Path directory) throws FileNotFoundException {
		...
		if (directory == null) {
			throw new NullPointerException("Path for the List method cannot be null");
		}
	...
	public boolean createFile(Path file)
		...
		if (!directoryMap.parentExists(file)) {
			throw new FileNotFoundException("The Parent Directory does not exist");
		}
	...
	public Path[] register(Storage client_stub, Command command_stub,
		...
		if (storageStubs.contains(client_stub) || commandStubs.contains(command_stub)) {
			throw new IllegalStateException("This Storage server already is registered");
		}
	...
	/**
	 * This represents mapping of the  Directory structure.
	 */
	private class DirectoryMap {
		...
		/**
		 * Adding a Path to the DirectoryMap
		 *
		 * @param path        to be added
		 * @param storageStub that is associated with Path
		 * @return true if Path was successfully added, or else false 
		 */
		...
		/**
		 * Adding a Path to the DirectoryMap associate with a directory. Similar to <code>addPath</code>
		 *
		 * @param path        to be added
...
/**
 * 
 *Contains methods to help the to fetch the path components
 *
 */
class Utilities {
	public static String[] getPathComponents(Path path) {
		String[] components;
		components = Arrays.stream(path.toString().split("/")).filter(e -> e.length() > 0)
				.toArray(String[]::new);
